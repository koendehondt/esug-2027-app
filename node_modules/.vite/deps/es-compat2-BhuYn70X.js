...
//#region node_modules/@ember/test-waiters/dist/token.js
/**
...
var Token = class {};
//#endregion
//#region node_modules/@ember/test-waiters/dist/index.js
var dist_exports = /* @__PURE__ */ __exportAll({
	_reset: () => _reset,
	_resetWaiterNames: () => _resetWaiterNames,
	buildWaiter: () => buildWaiter,
	getPendingWaiterState: () => getPendingWaiterState,
...
var TestWaiterImpl = class {
	...
	completedOperationsForTokens = /* @__PURE__ */ new WeakMap();
	completedOperationsForPrimitives = /* @__PURE__ */ new Map();
	...
	beginAsync(token = this.nextToken(), label) {
		this._register();
		if (this.items.has(token)) throw new Error(`beginAsync called for ${token} but it is already pending.`);
	...
	endAsync(token) {
		if (!this.items.has(token) && !this._getCompletedOperations(token).has(token)) throw new Error(`testWaiter.endAsync called with no preceding testWaiter.beginAsync call.
	...
	_getCompletedOperations(token) {
		...
		return !(type === "function") && !(token !== null && type === "object") ? this.completedOperationsForPrimitives : this.completedOperationsForTokens;
	}
};
/**
...
function buildWaiter(name) {
	warn(`The waiter name '${name}' is already in use`, !WAITER_NAMES.has(name), { id: "@ember/test-waiters.duplicate-waiter-name" });
	...
	warn(`You must provide a name that contains a descriptive prefix separated by a colon.
        ...
        You passed: ${name}`, WAITER_NAME_PATTERN.test(name), { id: "@ember/test-waiters.invalid-waiter-name" });
...
function wrapFunction(fn, label) {
	return function(...args) {
		...
		if (isThenable(result)) return waitForPromise(result, label);
		else if (isGenerator(result)) return waitForGenerator(result, label);
		else return result;
	};
}
	...
	return {
		next(...args) {
			...
			try {
				...
				hasErrored = false;
				if (val.done) stopWaiting();
				return val;
			} finally {
				if (hasErrored) stopWaiting();
			}
		},
		return(...args) {
			stopWaiting();
...
//#endregion
//#region node_modules/@embroider/macros/src/addon/es-compat2.js
function esCompat(m) {
...
//#endregion
export { hasPendingWaiters as a, getPendingWaiterState as i, buildWaiter as n, dist_exports as r, esCompat as t };
