import { d as debugToString, n as dict, o as unwrap, r as expect, t as StackImpl, u as isPresentArray } from "./collections-DPkjqeA3-BTZFUfs_.js";
import { i as hasCapability, o as InternalComponentCapabilities, s as MACHINE_MASK } from "./capabilities-CZkuOqqC-Bcpl3t5H.js";
import { a as encodeHandle, c as isSmallInt, d as EMPTY_STRING_ARRAY, h as reverse, l as EMPTY_ARRAY, o as encodeImmediate, p as enumerate, t as assert, y as assign } from "./assert-Zqc4wiAV-CoU6R4iL.js";
//#region node_modules/ember-source/dist/dev/packages/shared-chunks/vm-ops-ImHv0Wtg.js
function isMachineOp(value) {
	return value >= 0 && value <= 15;
}
//#endregion
//#region node_modules/ember-source/dist/dev/packages/@glimmer/encoder/index.js
var InstructionEncoderImpl = class {
	...
	encode(type, machine, ...args) {
		...
		for (const op of args) {
			if (typeof op === "number" && op > 2147483647) throw new Error(`Operand over 32-bits. Got ${op}.`);
			this.buffer.push(op);
		}
...
//#endregion
//#region node_modules/ember-source/dist/dev/packages/shared-chunks/opcodes-CplRyHl_.js
var opcodes = {
	Append: 1,
	TrustingAppend: 2,
	Comment: 3,
...
//#endregion
//#region node_modules/ember-source/dist/dev/packages/@glimmer/vm/index.js
var ContentType = {
	Component: 0,
	Helper: 1,
	String: 2,
...
//#endregion
//#region node_modules/ember-source/dist/dev/packages/shared-chunks/index-CyZiGiEI.js
...
function resolveComponent(resolver, constants, meta, [, expr, then]) {
	...
	if (expr[0] === opcodes.GetStrictKeyword) {
		...
		throw new Error(`Attempted to resolve a component in a strict mode template, but that value was not in scope: ${meta.symbols.upvars[expr[1]] ?? "{unknown variable}"}`);
	}
...
function resolveHelper(resolver, constants, meta, [, expr, then]) {
	...
	} else if (type === opcodes.GetStrictKeyword) then(lookupBuiltInHelper(expr, resolver, meta, constants, "helper"));
	else {
		let { symbols: { upvars }, owner } = assertResolverInvariants(meta);
		let name = unwrap(upvars[expr[1]]);
...
function resolveComponentOrHelper(resolver, constants, meta, [, expr, { ifComponent, ifHelper }]) {
	...
	} else if (type === opcodes.GetStrictKeyword) ifHelper(lookupBuiltInHelper(expr, resolver, meta, constants, "component or helper"));
	else {
		let { symbols: { upvars }, owner } = assertResolverInvariants(meta);
		let name = unwrap(upvars[expr[1]]);
		let definition = resolver?.lookupComponent?.(name, owner) ?? null;
...
function resolveOptionalComponentOrHelper(resolver, constants, meta, [, expr, { ifComponent, ifHelper, ifValue }]) {
	...
	if (type === opcodes.GetLexicalSymbol) {
		...
		ifValue(constants.value(definition));
	} else if (type === opcodes.GetStrictKeyword) ifHelper(lookupBuiltInHelper(expr, resolver, meta, constants, "value"));
	else {
		let { symbols: { upvars }, owner } = assertResolverInvariants(meta);
		let name = unwrap(upvars[expr[1]]);
		let definition = resolver?.lookupComponent?.(name, owner) ?? null;
...
function lookupBuiltInHelper(expr, resolver, meta, constants, type) {
	...
	if (helper === null) {
		assert(!meta.isStrictMode);
		throw new Error(`Attempted to resolve a ${type} in a strict mode template, but that value was not in scope: ${meta.symbols.upvars[expr[1]] ?? "{unknown variable}"}`);
	}
...
//#endregion
export { encodeOp as a, templateFactory as c, SwitchCases as i, ContentType as l, EncoderImpl as n, invokePreparedComponent as o, InvokeBareComponent as r, templateCacheCounters as s, CallDynamic as t, opcodes as u };
