...
import { f as scheduleOnce } from "./runloop-Ds2WWrox.js";
import { t as isEmpty } from "./is_empty-BfgYMUEp.js";
import { n as service, t as Service } from "./service-x9qJ7oMe.js";
import "./utils-CaX9Hpij.js";
import { n as _defineProperty, r as _initializerDefineProperty, t as _applyDecoratedDescriptor } from "./_rollupPluginBabelHelpers-9wjJaosZ-B_kcHwbT.js";
//#region node_modules/ember-page-title/dist/services/page-title.js
var _dec;
var _dec2;
var _class;
var _descriptor;
...
var RouterEvent = { ROUTE_DID_CHANGE: "routeDidChange" };
...
*/
var PageTitleService = (_dec = service("router"), _dec2 = service("-document"), _class = class PageTitleService extends Service {
	constructor(owner) {
		super(owner);
		...
		if (hasResolveRegistration(owner)) {
			...
			if (hasPageTitleConfig(config)) configKeys.forEach((key) => {
	...
	applyTokenDefaults(token) {
		...
		token.previous ??= null;
		token.next ??= null;
		if (token.separator == null) token.separator = defaultSeparator;
		if (token.prepend == null && defaultPrepend != null) token.prepend = defaultPrepend;
		if (token.replace == null && defaultReplace != null) token.replace = defaultReplace;
	}
	inheritFromPrevious(token) {
		const previous = token.previous;
		if (previous) {
			if (token.separator == null) token.separator = previous.separator;
			if (token.prepend == null) token.prepend = previous.prepend;
		}
	}
	push(token) {
		...
		if (tokenForId) {
			...
			token.previous = tokenForId.previous;
	...
	get sortedTokens() {
		...
		visible.forEach((token) => {
			if (token.front) frontGroups.unshift(token);
	...
	toString() {
		...
		for (let i = 0, len = tokens.length; i < len; i++) {
			...
			if (token.title) {
				...
				if (i + 1 < len) title.push(token.separator);
			}
		}
		return title.join("");
	}
	...
	_updateTitle() {
		...
		if (isFastBoot) this.updateFastbootTitle(toBeTitle);
		else
 /**
	...
	*/
	_validateExistingTitleElement() {
		...
		!(document.head.querySelectorAll("title").length <= 1) && assert("[ember-page-title]: Multiple title elements found. Check for other addons like ember-cli-head updating <title> as well.", document.head.querySelectorAll("title").length <= 1);
	}
	/**
	* Find token by id
	*
	...
	*/
	...
	updateFastbootTitle(toBeTitle) {
		...
		for (let i = 0; i < headChildNodes.length; i++) {
			...
			if (!node) continue;
			if (node.nodeName.toLowerCase() === "title") headElement.removeChild(node);
		}
