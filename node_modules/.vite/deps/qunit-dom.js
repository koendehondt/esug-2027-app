import { a as resolveDOMElements, i as resolveDOMElement, n as isDescriptor, o as resolveDescription, t as createDescriptor$1 } from "./es-BXqVk1dC.js";
//#region node_modules/qunit-dom/dist/es/index.js
function exists(options, message) {
	let expectedCount = null;
	if (typeof options === "string") message = options;
	else if (options) expectedCount = options.count;
	...
	} else throw new TypeError(`Unexpected Parameter: ${expectedCount}`);
}
function format$1(selector, num) {
	if (selector === "<unknown>") selector = "<not found>";
	if (num === void 0 || num === null) return `Element ${selector} exists`;
	else if (num === 0) return `Element ${selector} does not exist`;
	else if (num === 1) return `Element ${selector} exists once`;
	else if (num === 2) return `Element ${selector} exists twice`;
	else return `Element ${selector} exists ${num} times`;
}
...
function focused(message) {
	...
	if (!message) message = `Element ${expected} is focused`;
	this.pushResult({
		result,
		actual,
		expected,
...
function checked(message) {
	...
	if (!(isChecked || isNotChecked)) {
		...
		if (ariaChecked !== null) result = ariaChecked === "true";
	}
	let actual = result ? "checked" : "not checked";
	let expected = "checked";
	if (!message) message = `Element ${this.targetDescription} is checked`;
	this.pushResult({
		result,
		actual,
		expected,
...
function notChecked(message) {
	...
	if (!message) message = `Element ${this.targetDescription} is not checked`;
	this.pushResult({
		result,
		actual,
		expected,
...
function required(message) {
	let element = this.findTargetElement();
	if (!element) return;
	if (!(element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement || element instanceof HTMLSelectElement)) throw new TypeError(`Unexpected Element Type: ${element.toString()}`);
	let result = element.required === true;
	let actual = result ? "required" : "not required";
	let expected = "required";
	if (!message) message = `Element ${this.targetDescription} is required`;
	this.pushResult({
		result,
		actual,
		expected,
...
function notRequired(message) {
	...
	if (!message) message = `Element ${this.targetDescription} is not required`;
	this.pushResult({
		result,
		actual,
		expected,
...
function isValid(message, options = {}) {
	let element = this.findTargetElement();
	if (!element) return;
	if (!(element instanceof HTMLFormElement || element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement || element instanceof HTMLButtonElement || element instanceof HTMLOutputElement || element instanceof HTMLSelectElement)) throw new TypeError(`Unexpected Element Type: ${element.toString()}`);
	...
	if (!message) message = `Element ${this.targetDescription} is ${actual}`;
	this.pushResult({
		result,
		actual,
		expected,
...
function format(selector, num) {
	if (selector === "<unknown>") selector = "<not found>";
	if (num === void 0 || num === null) return `Element ${selector} is visible`;
	else if (num === 0) return `Element ${selector} is not visible`;
	else if (num === 1) return `Element ${selector} is visible once`;
	else if (num === 2) return `Element ${selector} is visible twice`;
	else return `Element ${selector} is visible ${num} times`;
}
...
function matchesSelector(elements, compareSelector) {
	return elements.filter((it) => !it.matches(compareSelector)).length;
}
...
function createDescriptor(target, rootElement) {
	if (typeof target === "string") {
		if (!rootElement) throw new Error("Cannot do selector-based queries without a root element");
		return createDescriptor$1(new SelectorData(target, rootElement));
	} else if (target instanceof Element) return createDescriptor$1(new ElementData(target));
	else if (target === null) return createDescriptor$1({
		element: null,
		description: "<unknown>"
	});
	else if (isDescriptor(target)) return target;
	else throw new TypeError(`Unexpected Parameter: ${target}`);
}
/**
* @namespace
*/
var DOMAssertions = class {
	testContext;
	/**
	* @ignore
	...
	*/
	hasAttribute(name, value, message) {
		...
		if (arguments.length === 1) value = { any: true };
	...
	*/
	hasAria(name, ...value) {
		if (value.length === 0) return this.hasAttribute(`aria-${name}`);
		else return this.hasAttribute(`aria-${name}`, ...value);
	}
	/**
	...
	*/
	hasClass(expected, message) {
		...
		if (expected instanceof RegExp) {
			let result = Array.prototype.slice.call(element.classList).some((className) => {
				return expected.test(className);
			});
			if (!message) message = `Element ${this.targetDescription} has CSS class matching ${expected}`;
			this.pushResult({
				result,
				actual,
				expected,
			...
			if (!message) message = `Element ${this.targetDescription} has CSS class "${expected}"`;
			this.pushResult({
				result,
				actual,
				expected,
	...
	*/
	doesNotHaveClass(expected, message) {
		...
		let actual = element.classList.toString();
		if (expected instanceof RegExp) {
			let result = Array.prototype.slice.call(element.classList).every((className) => {
				return !expected.test(className);
			});
			if (!message) message = `Element ${this.targetDescription} does not have CSS class matching ${expected}`;
			this.pushResult({
				result,
				actual,
			...
			let result = !element.classList.contains(expected);
			if (!message) message = `Element ${this.targetDescription} does not have CSS class "${expected}"`;
			this.pushResult({
				result,
				actual,
				expected: `not: ${expected}`,
	...
	*/
	hasPseudoElementStyle(selector, expected, message) {
		...
		if (expectedProperties.length <= 0) throw new TypeError(`Missing style expectations. There must be at least one style property in the passed in expectation object.`);
	...
	*/
	hasText(expected, message) {
		...
		if (expected instanceof RegExp) {
			...
			if (!message) message = `Element ${this.targetDescription} has text matching ${expected}`;
			this.pushResult({
				result,
				actual,
				expected,
			...
			if (!message) message = `Element ${this.targetDescription} has text "${expected}"`;
			this.pushResult({
				result,
				actual,
				expected,
		...
		} else throw new TypeError(`You must pass a string or Regular Expression to "hasText". You passed ${expected}.`);
		return this;
	}
	...
	*/
	doesNotIncludeText(text, message) {
		let element = this.findTargetElement();
		if (!element) return this;
		let result = collapseWhitespace(element.textContent || "").indexOf(text) === -1;
		let expected = `Element ${this.targetDescription} does not include text "${text}"`;
		let actual = expected;
		if (!result) actual = `Element ${this.targetDescription} includes text "${text}"`;
		if (!message) message = expected;
		this.pushResult({
			result,
			actual,
	...
	*/
	hasValue(expected, message) {
		let element = this.findTargetElement();
		if (!element) return this;
		if (arguments.length === 0) expected = { any: true };
		let value = element.value;
		if (expected instanceof RegExp) {
			let result = expected.test(value);
			let actual = value;
			if (!message) message = `Element ${this.targetDescription} has value matching ${expected}`;
			this.pushResult({
				result,
				actual,
				expected,
			...
			let actual = value;
			let result = actual === expected;
			if (!message) message = `Element ${this.targetDescription} has value "${expected}"`;
			this.pushResult({
				result,
				actual,
				expected,
	...
	*/
	includesValue(expected, message) {
		...
		if (!message) message = `Element ${this.targetDescription} includes value "${expected}"`;
		this.pushResult({
			result,
			actual,
			expected,
	...
	*/
	doesNotIncludeValue(expected, message) {
		...
		let actual = element.value;
		let result = !actual.includes(expected);
		if (!message) message = `Element ${this.targetDescription} does not include value "${expected}"`;
		this.pushResult({
			result,
			actual,
			expected,
	...
	*/
	matchesSelector(compareSelector, message) {
		...
		let expected;
		if (matchFailures === 0) {
			if (!message) message = singleElement ? `The element ${selectedByPart} also matches the selector ${compareSelector}.` : `${targets} elements, ${selectedByPart}, also match the selector ${compareSelector}.`;
			...
			if (!message) message = singleElement ? `The element ${selectedByPart} did not also match the selector ${compareSelector}.` : `${matchFailures} out of ${targets} elements ${selectedByPart} did not also match the selector ${compareSelector}.`;
	...
	*/
	doesNotMatchSelector(compareSelector, message) {
		...
		if (matchFailures === targets) {
			if (!message) message = singleElement ? `The element ${selectedByPart} did not also match the selector ${compareSelector}.` : `${targets} elements, ${selectedByPart}, did not also match the selector ${compareSelector}.`;
			actual = expected = message;
			this.pushResult({
				result: true,
				actual,
			...
			let difference = targets - matchFailures;
			if (!message) message = singleElement ? `The element ${selectedByPart} must not also match the selector ${compareSelector}.` : `${difference} elements out of ${targets}, ${selectedByPart}, must not also match the selector ${compareSelector}.`;
	...
	*/
	is(expected, message) {
		let element = this.findTargetElement();
		if (!element) return this;
		if (!(expected instanceof Element)) throw new TypeError(`You must pass an Element to "is". You passed ${expected}.`);
		let result = element === expected;
		let expectedDescription = elementToString(expected);
		let actualDescription = elementToString(element);
		if (!result && actualDescription === expectedDescription) actualDescription += " (a different element with the same description)";
		if (!message) message = `Element ${this.targetDescription} is the same element as ${expectedDescription}`;
		this.pushResult({
			result,
	...
	* @see {@link #is}
	*/
	isNot(unexpected, message) {
		let element = this.findTargetElement();
		if (!element) return this;
		if (!(unexpected instanceof Element)) throw new TypeError(`You must pass an Element to "isNot". You passed ${unexpected}.`);
		let result = element !== unexpected;
		let unexpectedDescription = elementToString(unexpected);
		let actualDescription = result ? elementToString(element) : `${elementToString(element)} (the same element)`;
		if (!message) message = `Element ${this.targetDescription} is not the same element as ${unexpectedDescription}`;
		this.pushResult({
			result,
			actual: actualDescription,
	...
	*/
	hasTagName(tagName, message) {
		...
		let expected;
		if (!element) return this;
		if (typeof tagName !== "string") throw new TypeError(`You must pass a string to "hasTagName". You passed ${tagName}.`);
		actual = element.tagName.toLowerCase();
		expected = tagName.toLowerCase();
		if (actual === expected) {
			if (!message) message = `Element ${this.targetDescription} has tagName ${expected}`;
			this.pushResult({
				result: true,
				actual,
				expected,
			...
			if (!message) message = `Element ${this.targetDescription} does not have tagName ${expected}`;
			this.pushResult({
				result: false,
				actual,
				expected,
	...
	*/
	doesNotHaveTagName(tagName, message) {
		...
		let expected;
		if (!element) return this;
		if (typeof tagName !== "string") throw new TypeError(`You must pass a string to "doesNotHaveTagName". You passed ${tagName}.`);
		actual = element.tagName.toLowerCase();
		expected = tagName.toLowerCase();
		if (actual !== expected) {
			if (!message) message = `Element ${this.targetDescription} does not have tagName ${expected}`;
	...
	*/
	hasHtml(expected, message) {
		...
		if (expected instanceof RegExp) {
			...
			if (!message) {
				if (result) message = `Element ${this.targetDescription} has html matching ${expected}`;
				else message = `Element ${this.targetDescription} does not have html matching ${expected}`;
			}
			this.pushResult({
				result,
				actual,
			...
			let result = actual === expected;
			if (!message) {
				if (result) message = `Element ${this.targetDescription} has html "${expected}"`;
				else message = `Element ${this.targetDescription} does not have html "${expected}"`;
			}
			this.pushResult({
				result,
				actual,
		...
		} else throw new TypeError(`You must pass a string or Regular Expression to "hasHtml". You passed ${expected}.`);
		return this;
	}
	/**
	* Assert that the html of the {@link HTMLElement} or an {@link HTMLElement}
	...
	*/
	doesNotHaveHtml(expected, message) {
		let element = this.findTargetElement();
		if (!element) return this;
		if (typeof expected !== "string") throw new TypeError(`You must pass a string to "doesNotHaveHtml". You passed ${expected}.`);
		let actual = element.innerHTML;
		if (actual !== expected) {
			if (!message) message = `Element ${this.targetDescription} does not have html "${expected}"`;
			this.pushResult({
				result: true,
				actual,
				expected,
			...
			if (!message) message = `Element ${this.targetDescription} has html "${expected}"`;
			this.pushResult({
				result: false,
				actual,
				expected,
	...
	*/
	doesNotIncludeHtml(html, message) {
		let element = this.findTargetElement();
		if (!element) return this;
		let result = collapseWhitespace(element.innerHTML || "").indexOf(html) === -1;
		let expected = `Element ${this.targetDescription} does not include html "${html}"`;
		let actual = expected;
		if (!result) actual = `Element ${this.targetDescription} includes html "${html}"`;
		if (!message) message = expected;
		this.pushResult({
			result,
			actual,
	...
	*/
	get selectedBy() {
		if (this.wasPassedElement) return "passed";
		else if (resolveDOMElement(this.descriptor)) return `selected by ${this.targetDescription}`;
		else return "selected by null";
	}
};
var _getRootElement = () => null;
...
function install(assert) {
	assert.dom = function(target, rootElement) {
		if (!isValidRootElement(rootElement)) throw new Error(`${rootElement} is not a valid root element`);
		rootElement = rootElement || this.dom.rootElement || getRootElement();
		if (arguments.length === 0) target = rootElement instanceof Element ? rootElement : null;
...
function setup(assert, options = {}) {
	install(assert);
	overrideRootElement(typeof options.getRootElement === "function" ? options.getRootElement : () => document.querySelector("#ember-testing"));
}
//#endregion
