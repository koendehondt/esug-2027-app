...
function _MUTATE(target, receiver, prop, args, _SIGNAL) {
  ...
  switch (prop) {
    ...
    case 'push':
      {
        ...
        assertNoDuplicates(collection, target, currentState => currentState.push(...newValues));
        {
          // dedupe
    ...
    case 'unshift':
      {
        const newValues = extractIdentifiersFromRecords(args);
        assertNoDuplicates(collection, target, currentState => currentState.unshift(...newValues));
        {
          // dedupe
          const seen = new Set(target);
          const unique = new Set();
    ...
    case 'splice':
      {
        ...
        if (start === 0 && deleteCount === collection[Context].source.length) {
          ...
          assertNoDuplicates(collection, target, currentState => currentState.splice(start, deleteCount, ...newValues));
          {
            // dedupe
...
function assertNoDuplicates(collection, target, callback, reason) {
  collection[Context].features.identifier;
  const state = target.slice();
  callback(state);
  if (state.length !== new Set(state).size) {
    state.filter((currentValue, currentIndex) => state.indexOf(currentValue) !== currentIndex);
  }
}
